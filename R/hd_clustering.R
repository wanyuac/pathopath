...
hd_clustering <- function(mat = NULL, thresholds = c(5L, 10L, 15L, 20L, 50L), excl_subjects = "") {
    ...
    if (length(thresholds) > 0) {
        ...
        print("Warning: since thresholds are not specified, I will take a single threshold zero.")
        thresholds <- 0L
    }
    ...
    rownames(cls) <- NULL  # Remove row names
